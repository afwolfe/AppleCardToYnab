...
argparser.add_argument("-f", action="store_true", help="Force overwriting of outputPath.")
cardvision = argparser.add_argument_group("cardvision")
cardvision.add_argument("--cardVisionPath", default="CardVision", help="The path to use for CardVision. Defaults to 'CardVision/'", required=False)
cardvision.add_argument("--imagePath", default="screenshots/", help="The path to the folder containing the screenshots for CardVision. Defaults to 'screenshots'", required=False)
cardvision.add_argument("--outputPath", default="transactions.csv", help="The file to write the transaction data from CardVision. Defaults to 'transactions.csv'", required=False)
ynab = argparser.add_argument_group("ynab")
ynab.add_argument("--budgetId", default=os.environ.get("YNAB_BUDGET_ID"), help="The YNAB budget ID to write to, defaults to YNAB_BUDGET_ID in env.", required=False)
ynab.add_argument("--apiKey", default=os.environ.get("YNAB_API_KEY"), help="The YNAB API key to use to, defaults to YNAB_API_KEY in env.", required=False)
ynab.add_argument("--accountId", default=os.environ.get("YNAB_APPLE_CARD_ACCOUNT_ID"), help="The YNAB account ID of the Apple Card, defaults to YNAB_APPLE_CARD_ACCOUNT_ID in env. Will attempt to read find account with 'Apple' in budget if not specified.", required=False)
...
def parse_cardvision_csv(csv_file: os.PathLike) -> List[save_transaction.SaveTransaction]:
